...
For hosted providers (Anthropic, OpenAI, Gemini, etc.), run `goose configure` and follow the prompts to select your provider and enter your API key.
...
brew trust dosu-ai/dosu
brew install dosu
dosu login
```

:::note
Homebrew on Linux requires trusting third-party taps with `brew trust <tap>` before installing formulae.
:::

Goose connects to Dosu via MCP using its HTTP transport. After logging in, use the manual config path to get your connection details:

```bash
dosu mcp add manual --show-secret
```

This prints your Dosu MCP endpoint URL and API key. Add them to `~/.config/goose/config.yaml` under `extensions`:
...
extensions:
  dosu:
    ...
    uri: <Endpoint from dosu mcp add manual>
    headers:
      X-Dosu-API-Key: <Header value from dosu mcp add manual>
    timeout: 30
```

Once added, you can ask Goose questions that draw on your org's docs alongside the live system data from `linux-mcp-server`.

If you use other AI tools such as GitHub Copilot CLI, Claude Code, Cursor, or VS Code, Dosu can configure them directly:

```bash
dosu mcp add copilot
# or: dosu mcp add claude, dosu mcp add cursor, dosu mcp add vscode
```
